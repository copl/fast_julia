...
# - replace object methods with functions (gives better speed and debugging)
#############################################################
...
function display_progress(itr::Int64, alpha::Float64, gamma::Float64, residuals::class_residuals, vars::class_variables, direction::class_variables, delta::Float64, ls_num_trials::Int64, num_facs::Int64, settings::class_settings)
	try
		output = @sprintf("%s %2.1e %2.1e | %2.1e %2.1e | %2.1e %2.1e %2.1e %2.1e %2.1e | %2.1e %2.1e %s %s \n", rpad(string(itr),3), alpha, gamma, vars.tau(), vars.kappa(), residuals.scaled_mu, residuals.r_G_norm_scaled, residuals.r_P_norm_scaled, residuals.r_D_norm_scaled, residuals.func_val, delta, alpha * norm(direction.v,1), rpad(string(ls_num_trials),2), rpad(string(num_facs),2) )
...
function homogeneous_algorithm(qp::class_quadratic_program, vars::class_variables, settings::class_settings)
	...
	try
		...
		for it = 1:settings.max_it
			...
			used_delta, num_facs = ipopt_style_inertia_correction(newton_solver, vars, qp, settings)
			total_factorizations += num_facs;
			#used_delta = newton_solver.update_system(vars)

			vars, alpha, gamma = hybrid_mu_strategy(newton_solver, vars, qp, settings, used_delta)
